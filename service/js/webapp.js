var SPWebApp = {
    ...
    InitEvent: function() {
        ...
        if(typeof(SPWebApp.m_objWndChild.SPUrlMgt) == 'undefined'){
            
            SPWebApp.m_objWndChild = document.getElementById("Iframe_Content").contentWindow;
            // SPWebApp.m_objWndChild.SPUrlMgt = {
            //     m_objOnline : {
            //         totalurl : 0
            //     },
            //     m_arrCBox : [],
    ...
    OpenAddUrl: function() {
        ...
        // console.log(SPWebApp.m_objWndChild);
    ...
    OpenOptimize: function(objItem) {
        ...
        if ((undefined != objOptimize.odrid) && (0 != objOptimize.status)) {
            $("#Span_SetFUrl_Number", objDiv).html(100);
        } else {
            $("#Span_SetFUrl_Number", objDiv).html(100);
        }
    ...
    AddFurlToList: function() {
        ...
        if ($("#Manage_My_Submit").data("boptimize")) {
            if (100 == objSelect.children().length) {
                alert("您的来源网址数量已经用完！");
                return;
            }
        } else {
            if (100 == objSelect.children().length) {
                alert("目前最多只能设置100个来源网址！");
                return;
            }
        }
        var furl = jObj.val();
