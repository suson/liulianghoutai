...
require_once("./alipay/myAlipaycfg.php");
	...
	class DB {
		...
		public function addUrlOnline( $url, $name, $tid, $level){
			...
			$odrs=array();
			$urltype=SVC_IP8;
			$qAddUrl="INSERT INTO url(userid,url,name,rtime,ltime,urltype,free,furls) VALUES('".$this->userid."','$url','$name',now(),now(),$urltype,1,'');";
		...
		public function getControUrl($urlid){
			$qFlow="SELECT hrs FROM ".TB_FLOWER." WHERE cid='$urlid';";
			$result=mysql_query($qFlow,$this->link);
			$num=mysql_num_rows($result);
			if(0 ==$num){
				$this->response['hrs']=explode('|',"100|100|100|100|100|100|100|100|100|100|100|100|100|100|100|100|100|100|100|100|100|100|100|100");
				$this->response['error']=0;
				$this->response['cid']=$urlid;
				return $this->response;
			}else{
				$row=mysql_fetch_array($result,MYSQL_ASSOC);
				$this->response['hrs']=explode('|',$row['hrs']);
				$this->response['cid']=$urlid;
				$this->response['error']=0;
				return $this->response;
			}
